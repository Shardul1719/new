...
const About = () => {
  ...
  // Track when heading first comes into view
  React.useEffect(() => {
    const unsubscribe = headingProgress.on("change", (v) => {
      if (v > 0.1 && !hasAnimated) {
        setHasAnimated(true);
      }
    });
    return () => unsubscribe();
  }, [headingProgress, hasAnimated]);
                  ...
                  {!isMobile
                    ? characters.map((char, idx) => (
                        <Character
                          ...
                          index={idx}
                          centerIndex={centerIndex}
                          // Freeze animation after it finishes once
                          scrollYProgress={
                            hasAnimated ? { current: 1 } : headingProgress
                          }
                          isMobile={isMobile}
                        />
                      ))
          ...
          >
            {features.map((feature, index) => (
              <CardAnimated
                ...
                index={index}
                total={features.length}
                // Freeze cards too after first play
                scrollYProgressCards={
                  hasAnimated ? { current: 1 } : cardsProgress
                }
                isMobile={isMobile}
              >
                <Card className="p-6 hover:shadow-lg transition-all duration-300 hover:scale-105 border-border h-full">
